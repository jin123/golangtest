...
func main() {
	...
	go func(t *time.Ticker) {
		defer wg.Done()
		for {
			//fmt.Println("time.Ticker_t_c=", t.C)
			<-t.C
			fmt.Println("get_ticker_times=", time.Now().Format("2006-01-02 15:04:05"))
		}
	}(ticker1)
	...
	go func(t *time.Timer) {
		defer wg.Done()
		for {
			//fmt.Println("time.Timer_t_c=", t.C)
			<-t.C
			fmt.Println("get_timer_times", time.Now().Format("2006-01-02 15:04:05"))
